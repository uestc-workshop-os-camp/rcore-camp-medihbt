...
pub mod mmap_handle {
    ...
    fn _check_vaddr_if_mapped(mset: &MemorySet, start: usize, end: usize)-> bool {
        ...
        for vp in vpns {
            if mset.has_vpn(vp) { return true; }
        }
        false
    }
    ...
    pub fn do_mmap(start: usize, len: usize, prot: usize)-> isize
    {
         ...
         * will be registered   */
        update_current_tcb(|_tid, tcb| {
            let mset = &mut tcb.memory_set;
    ...
    pub fn do_munmap(start: usize, len: usize)-> isize {
        ...
        update_current_tcb(|_, tcb| {
            if !tcb.memory_set.unmap_range(pbegin, npages) {
                retval = -1;
            }
        });
