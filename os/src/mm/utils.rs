...
pub mod mmap_handle {
    ...
    pub fn do_mmap(start: usize, len: usize, prot: usize)-> isize
    {
         ...
         * will be registered   */
        update_current_tcb(|pcb, _tcbi| {
            let mset = &mut pcb.inner_exclusive_access().memory_set;
    ...
    pub fn do_munmap(start: usize, len: usize)-> isize {
        ...
        update_current_tcb(|pcb, _tcbi| {
            if pcb.inner_exclusive_access().memory_set.unmap_range(pbegin, npages) { 0 } else { -1 }
        })
    }
}